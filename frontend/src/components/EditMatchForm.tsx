...
interface EditMatchFormProps {
  onSubmit: (match: Partial<CreateMatchRequest>) => Promise<void>;
  onCancel: () => void;
  players: Player[];
  decks: Deck[];
  initialData: Match;
...
function EditMatchForm({ onSubmit, onCancel, players, decks, initialData }: EditMatchFormProps) {
  ...
  // Initialize form with existing match data
  useEffect(() => {
    if (initialData) {
      // Set selected players
      const playerIds = initialData.players.map(p => p.player_id);
      setSelectedPlayerIds(playerIds);

      // Set player-deck mapping
      const deckMap: Record<string, string> = {};
      initialData.players.forEach(p => {
        deckMap[p.player_id] = p.deck_id;
      });
      setPlayerDeckMap(deckMap);

      // Set winner
      setWinnerId(initialData.winner_player_id);

      // Set match date
      setMatchDate(initialData.match_date.split('T')[0]); // Remove time portion
    }
  }, [initialData]);
    ...
    <div className="fixed inset-0 bg-black/70 flex items-start md:items-center justify-center z-[1000] p-3 md:p-6" onClick={onCancel}>
      <div className="bg-gradient-card rounded-[16px] md:rounded-[12px] p-0 md:p-8 w-full max-w-full md:max-w-[700px] shadow-[0_4px_16px_rgba(0,0,0,0.2)] min-h-[calc(100vh-24px)] md:min-h-0 max-h-[calc(100vh-24px)] md:max-h-none flex flex-col md:block overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="mb-0 md:mb-6 flex items-center justify-between sticky md:static top-0 bg-gradient-card border-b border-[#2C2E33] md:border-b-0 p-4 px-5 md:p-0 z-10 flex-shrink-0">
          <h2 className="text-white m-0 text-xl md:text-2xl font-semibold flex-1">Edit Match</h2>
        </div>
